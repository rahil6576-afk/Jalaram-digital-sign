...
export default function FloatingWhatsApp() {
  ...
  return (
    <a
      ...
      className="fixed bottom-5 right-4 sm:bottom-6 sm:right-6 z-[9999] w-14 h-14 flex items-center justify-center bg-[#25D366] hover:bg-[#20ba59] text-white rounded-full shadow-2xl shadow-[#25D366]/50 hover:scale-110 active:scale-95 transition-transform duration-200 focus:outline-none focus:ring-4 focus:ring-[#25D366]/30 group"
    >
      {/* Radar ping */}
      <span className="absolute inset-0 rounded-full bg-[#25D366] animate-ping opacity-30 pointer-events-none" />

      {/* Tooltip */}
      <span className="absolute right-full mr-3 px-3.5 py-1.5 bg-gray-950/95 text-white text-xs font-semibold rounded-full shadow-2xl opacity-0 group-hover:opacity-100 transition-all duration-200 pointer-events-none whitespace-nowrap backdrop-blur-md border border-white/10 hidden sm:block">
